...
- 未購入かつ価格設定済みの写真の購入対象追加ボタン
- 購入済み写真の購入済み表示（購入対象には追加不可）
- `price` が `null` の写真は価格未設定として表示し、購入対象には追加不可
- ページネーション
...
- まとめて購入の選択状態は画面ローカルではなくクライアント状態として保持し、写真詳細へ遷移して戻っても選択を維持する。
- 絞り込みフォームで再検索した場合は、検索対象の変化による誤購入を避けるため選択状態をリセットする。
